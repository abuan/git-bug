...
func SelectBug(repo *cache.RepoCache, id entity.Id) error {
	selectPath := selectFilePath(repo)
...
func ClearBug(repo *cache.RepoCache) error {
	selectPath := selectFilePath(repo)

	return os.Remove(selectPath)
}

func selectedBug(repo *cache.RepoCache) (*cache.BugCache, error) {
	selectPath := selectFilePath(repo)
